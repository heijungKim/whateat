...
1. **냉장고 사진 촬영** → AI 비전으로 식재료 자동 인식 (잘못 인식된 재료는 수동 수정 가능)
   - 🆓 무료: [Google AI Studio](https://aistudio.google.com/apikey)에서 Gemini 키 발급 (카드 등록 불필요)
   - 💎 유료(정확도↑): Claude API 키 — 키 형식(AIza / sk-ant-)으로 자동 판별
   - 키 없이도 재료 직접 입력으로 전체 기능 사용 가능
